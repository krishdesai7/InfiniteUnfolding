...
X_train_truth, X_temp_truth, X_train_reco, X_temp_reco, Y_train, Y_temp = train_test_split(
    xvals_truth, xvals_reco, yvals, test_size=0.3, random_state=42)
...
def build_critic(input_shape, disc_model_width=75, disc_model_depth=5, dropout_rate=0.2):
    ...
    for _ in range(disc_model_depth):
        ...
        x = tf.keras.layers.LeakyReLU(negative_slope=0.2)(x)
        x = tf.keras.layers.Dropout(dropout_rate)(x)
        x = tf.keras.layers.LayerNormalization()(x)  # Batch normalization still applied after dropout
    
    # Apply spectral normalization to the final output layer
    outputs = tf.keras.layers.SpectralNormalization(
                tf.keras.layers.Dense(1, activation=None)
            )(x)  # Linear activation for the critic output
...
val_dataset = tf.data.Dataset.from_tensor_slices((X_val_reco, X_val_truth, Y_val)).batch(batch_size)
...
for epoch in range(n_epochs):
    ...
    for X_val_detector_batch, X_val_particle_batch, Y_val_batch in val_dataset:
        # Validation critic loss
        model_generator.trainable = False
        model_critic.trainable = True
        W_val_batch = model_generator.predict(X_val_particle_batch, verbose=0)
        W_val_batch = tf.where(Y_val_batch == 1, 1.0, tf.squeeze(W_val_batch))
        val_d_loss = train_step_critic(X_val_detector_batch, Y_val_batch, W_val_batch)
        val_critic_losses.append(val_d_loss.numpy())

        # Validation generator loss
        alt_val_indices = tf.where(Y_val_batch == 0)
        X_val_particle_alt = tf.gather_nd(X_val_particle_batch, alt_val_indices)
        X_val_detector_alt = tf.gather_nd(X_val_detector_batch, alt_val_indices)
        Y_val_alt = tf.gather_nd(Y_val_batch, alt_val_indices)
        val_g_loss = train_step_gan(X_val_particle_alt, X_val_detector_alt, Y_val_alt)
        val_generator_losses.append(val_g_loss.numpy())

    # Average loss over the epoch
    avg_c_loss = np.mean(critic_losses[-n * len(train_dataset):])
    avg_g_loss = np.mean(generator_losses[-m * len(train_dataset):])
    avg_val_c_loss = np.mean(val_critic_losses)
    avg_val_g_loss = np.mean(val_generator_losses)
...
np.savez_compressed('WGAN_training_metrics_compressed.npz', crit_loss_avg=crit_loss_avg, gen_loss_avg=gen_loss_avg,
                    avg_val_c_loss = avg_val_c_loss, avg_val_g_loss = avg_val_g_loss,
                    baseline=baseline, performance_metric=performance_metric,
                    X_test_truth = X_test_truth, Y_test = Y_test)
